...
exports.run = function(query) {
	...
	program_results = hyperparams_programs_results[hyperparams_name][program_name];
    ...
    Object.keys(hyperparams_programs_results).forEach(hyperparams => {
	programs_results_lists[hyperparams] = [];
	Object.keys(hyperparams_programs_results[hyperparams]).forEach(program => {
	    program_results = hyperparams_programs_results[hyperparams][program];
	    programs_results_lists[hyperparams][program] = [];
	    Object.keys(program_results).forEach(output => {
		programs_results_lists[hyperparams][program].push({
		    "output": output,
	    ...
	    programs_results_lists[hyperparams][program].sort((a,b) => {
		return b["satisfaction"] - a["satisfaction"];
	    });
	});
    });
    console.log(programs_results_lists)
    doc = `
<html>
  <head>
    <style>
...
`;
    Object.keys(programs_results_lists).forEach(hyperparams => {
	doc += `
    <h3>${hyperparams}</h3>
`
	Object.keys(programs_results_lists[hyperparams]).forEach(program => {
      ...
      <table>
        ...
        <tr>
`;
	    program_results = programs_results_lists[hyperparams][program];
