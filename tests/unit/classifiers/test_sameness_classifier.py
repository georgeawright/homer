...
def test_classify_labels_with_different_concept():
    start = Label(Mock(), Mock(), Mock(), 1.0)
    end = Label(Mock(), Mock(), Mock(), 0.0)
    classifier = SamenessClassifier()
...
def test_classify_chunks_with_same_labels():
    ...
    correspondence = Correspondence(
        start_label, end_label, Mock(), Mock(), sameness_concept, Mock(), Mock(), 1.0
    )
...
def test_classify_relations_with_same_arguments():
    ...
    start_relation = Relation(
        start_relation_start, start_relation_end, relation_parent_concept, Mock(), 1.0
    )
    end_relation = Relation(
        end_relation_start, end_relation_end, relation_parent_concept, Mock(), 1.0
    )
