...
def test_successful_creates_view_and_spawns_follow_up_and_same_view_cannot_be_recreated(
    ...
    builder = DiscourseViewBuilder.spawn(
        parent_id, bubble_chamber, target_spaces, urgency
    )
    builder.run()
    assert CodeletResult.SUCCESS == builder.result
    assert hasinstance(builder.child_structures, DiscourseView)
    assert isinstance(builder.child_codelets[0], DiscourseViewEvaluator)
    builder = DiscourseViewBuilder.spawn(
        parent_id, bubble_chamber, target_spaces, urgency
    )
    builder.run()
