...
def bubble_chamber():
    ...
    relation = Relation(
        Mock(), Mock(), correspondence_concept, suggest_concept, None, None, 1
    )
...
def test_gives_high_confidence_for_compatible_chunks_and_spawns_follow_up(
    ...
    suggester = CorrespondenceSuggester.spawn(
        "",
        bubble_chamber,
        {
    ...
    assert same_concept == suggester.parent_concept
    assert suggester.confidence == 1
    assert isinstance(suggester.child_codelets[0], CorrespondenceBuilder)
