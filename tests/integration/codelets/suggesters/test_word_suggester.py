...
def bubble_chamber():
    ...
    relation = Relation(Mock(), Mock(), word_concept, suggest_concept, None, None, 1)
...
def template_chunk(
    ...
    input_to_template_chunk_correspondence._activation = 1
...
def test_gives_high_confidence_for_content_word_and_spawns_follow_up(
    ...
    parent_id = ""
    urgency = 1.0
    suggester = WordSuggester.spawn(
        parent_id,
        bubble_chamber,
        {"target_view": target_view, "target_word": template_slot_word},
        urgency,
    )
    suggester.run()
...
def test_gives_high_confidence_for_function_word_and_spawns_follow_up(
    ...
    suggester = WordSuggester.spawn(
        parent_id,
        bubble_chamber,
        {"target_view": target_view, "target_word": template_function_word},
        urgency,
    )
    suggester.run()
    assert CodeletResult.SUCCESS == suggester.result
